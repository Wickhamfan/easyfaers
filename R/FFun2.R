...
#'
#' @examples
FFun2 = function (drug_PS, drug_concomitant, age, gender, weight, agent,
                   country, drugEffect, year, illnesses, path)
{
  ...
  target.drug <- inter.drug[(DRUGNAME %in% drug_PS | prod_ai %in%
                               drug_PS) & (ROLE_COD %in% drugEffect)]
  target.drug <- inter.drug[unique(target.drug$primaryid)]
  target.drug <- target.drug[unique(target.drug[DRUGNAME %in%
                                                  drug_concomitant | prod_ai %in% drug_concomitant]$primaryid)]
  ...
  T2 <- T2 %>% dplyr::select(primaryid, DRUG_SEQ, DRUGNAME,
                             ROLE_COD)
  T1 <- left_join(T1, T2, by = "primaryid")
  T1 <- left_join(T1, T3, by = c("primaryid", "DRUG_SEQ"))
  ...
  T2 <- na.omit(T2)
  write.xlsx(T1, file = file.path(path, "联合用药不良反应诱发时间.xlsx"))
  write.xlsx(T2, file = file.path(path, "联合用药不良反应诱发时间(每个PT的不良反应诱发时间).xlsx"))
  ...
  write.xlsx(ok2, file = file.path(path, "联合用药不良反应信号(PT).xlsx"))
  ...
  if (NROW(okk2)) {
    ...
    okk2[, `:=`(EBGM, (a * (a + b + c + d))/((a + b) * (a +
                                                          c)))]
    okk2[, `:=`(EBGM05, exp(log(EBGM) - 1.64 * (sqrt(1/a +
                                                       1/b + 1/c + 1/d))))]
  ...
  okk2 <- okk2 %>% dplyr::mutate_if(is.numeric, round, digits = 2)
  ...
  okk2$"IC(95%下限)" <- paste(okk2$IC2, "(", okk2$C025,
                            ")")
  write.xlsx(okk2, file = file.path(path, "联合用药不良反应信号(soc).xlsx"))
  ...
  okkk2$"IC(95%下限)" <- paste(okkk2$IC2, "(", okkk2$C025,
                             ")")
  write.xlsx(okkk2, file = file.path(path, "联合用药不良反应信号(hlt).xlsx"))
  ...
  okkkk2$"IC(95%下限)" <- paste(okkkk2$IC2, "(", okkkk2$C025,
                              ")")
  write.xlsx(okkkk2, file = file.path(path, "联合用药不良反应信号(hlgt).xlsx"))
  df1 <- datalib1.demo[, `:=`(GetDataYear, dplyr::case_when(41 <=
  ...
  ggsave(file.path(path, "联合用药上报年份图.png"),
         p1, width = 20, height = 8.27, dpi = 600)
  write.xlsx(df1, file = file.path(path, "联合用药季度上报频数统计.xlsx"))
  ...
  df2 <- data.frame(df2)
  write.xlsx(df2, file = file.path(path, "联合用药结局指标频数统计.xlsx"))
  ...
  datalib1.demo <- datalib1.demo %>% dplyr::select(AGE, WT,
                                                   SEX, OCCP_COD, REPORTER_COUNTRY, GetDataYear) %>% dplyr::mutate(GetDataYear = dplyr::case_when(41 <=
                                                                                                                                                  ...
                                                                                                                                                  91 <= GetDataYear & GetDataYear <= 94 ~ "2009年", 101 <=
                                                                                                                                                    ...
                                                                                                                                                    "missing")) %>% dplyr::mutate(AGE = dplyr::case_when(AGE <=
                                                                                                                                                                                                           17 ~ "≤17岁", 17 < AGE & AGE <= 64 ~ "18～64岁",
                                                                                                                                                                                                         65 <= AGE & AGE <= 85 ~ "65～85岁", 85 < AGE ~ "≥86岁")) %>%
    dplyr::mutate(WT = dplyr::case_when(WT <= 49 ~ "＜50 kg",
                                        50 < WT & WT <= 100 ~ "50～100 kg", 100 < WT ~
                                          "＞100 kg")) %>% dplyr::mutate(SEX = dplyr::case_when(SEX ==
  ...
  datalib1.demo <- table1(~SEX + WT + AGE + OCCP_COD + REPORTER_COUNTRY,
                          data = datalib1.demo)
  datalib1.demo <- data.frame(datalib1.demo)
  write.xlsx(datalib1.demo, file = file.path(path, "联合用药基线信息整理汇总.xlsx"))
  target.drug <- inter.drug[(DRUGNAME %in% drug_PS | prod_ai %in%
                               drug_PS) & (ROLE_COD %in% drugEffect)]
  target.drug <- inter.drug[unique(target.drug$primaryid)]
  target.drug <- target.drug[!unique(target.drug[DRUGNAME %in%
  ...
  T2 <- merge(T4, T1, by = "primaryid", all.x = T)
  T2 <- na.omit(T2)
  write.xlsx(T1, file = file.path(path, "单独用药不良反应诱发时间.xlsx"))
  write.xlsx(T2, file = file.path(path, "单独用药不良反应诱发时间(每个PT的不良反应诱发时间).xlsx"))
  ok <- datalib2.reac[, .(a = as.numeric(.N)), PT]
  ok[, `:=`(b, as.numeric(NROW(datalib2.reac)) - a)]
  ok1 <- inter.reac[, .(ac = as.numeric(.N)), PT]
  ok1[, `:=`(n, as.numeric(NROW(inter.reac)))]
  ...
  ok2 <- data.frame(ok2)
  ok2 <- ok2 %>% select(PT, a, b, c, d, ROR, RORL, RORU, PRR,
                        XX, EBGM, EBGM05, IC2, C025, pt_name_cn, soc_name_en,
                        soc_name_cn) %>% arrange(-a)
  ok2$"ROR(95%Cl)" <- paste(ok2$ROR, "(", ok2$RORL, "-", ok2$RORU,
                            ")")
  ok2$"PRR(卡方值)" <- paste(ok2$PRR, "(", ok2$XX, ")")
  ok2$"EBGM(95%下限)" <- paste(ok2$EBGM, "(", ok2$EBGM05,
                             ")")
  ok2$"IC(95%下限)" <- paste(ok2$IC2, "(", ok2$C025, ")")
  write.xlsx(ok2, file = file.path(path, "单独用药不良反应信号(PT).xlsx"))
  okk <- datalib2.reac[, .(a = as.numeric(.N)), soc_name_cn]
  okk[, `:=`(b, as.numeric(NROW(datalib2.reac)) - a)]
  okk1 <- inter.reac[, .(ac = as.numeric(.N)), soc_name_cn]
  okk1[, `:=`(n, as.numeric(NROW(inter.reac)))]
  ...
  okk2$"IC(95%下限)" <- paste(okk2$IC2, "(", okk2$C025,
                            ")")
  write.xlsx(okk2, file = file.path(path, "单独用药不良反应信号(soc).xlsx"))
  okkk <- datalib2.reac[, .(a = as.numeric(.N)), hlt_name_cn]
  okkk[, `:=`(b, as.numeric(NROW(datalib2.reac)) - a)]
  okkk1 <- inter.reac[, .(ac = as.numeric(.N)), hlt_name_cn]
  okkk1[, `:=`(n, as.numeric(NROW(inter.reac)))]
  ...
  okkk2$"IC(95%下限)" <- paste(okkk2$IC2, "(", okkk2$C025,
                             ")")
  write.xlsx(okkk2, file = file.path(path, "单独用药不良反应信号(hlt).xlsx"))
  okkkk <- datalib2.reac[, .(a = as.numeric(.N)), hlgt_name_cn]
  okkkk[, `:=`(b, as.numeric(NROW(datalib2.reac)) - a)]
  okkkk1 <- inter.reac[, .(ac = as.numeric(.N)), hlgt_name_cn]
  okkkk1[, `:=`(n, as.numeric(NROW(inter.reac)))]
  ...
  okkkk2$"IC(95%下限)" <- paste(okkkk2$IC2, "(", okkkk2$C025,
                              ")")
  write.xlsx(okkkk2, file = file.path(path, "单独用药不良反应信号(hlgt).xlsx"))
  df1 <- datalib1.demo[, `:=`(GetDataYear, dplyr::case_when(41 <=
                                                              GetDataYear & GetDataYear <= 44 ~ "2004年", 51 <= GetDataYear &
                                                              GetDataYear <= 54 ~ "2005年", 61 <= GetDataYear & GetDataYear <=
                                                              64 ~ "2006年", 71 <= GetDataYear & GetDataYear <= 74 ~
  ...
  p1 <- ggplot(df1, aes(x = GetDataYear, y = Number)) + geom_bar(stat = "identity",
                                                                 width = 0.4) + xlab("") + theme_bw()
  ggsave(file.path(path, "单独用药上报年份图.png"),
         p1, width = 20, height = 8.27, dpi = 600)
  write.xlsx(df1, file = file.path(path, "单独用药季度上报频数统计.xlsx"))
  df2 <- data.frame(datalib2.outc)
  df2 <- apply(df2, 2, function(x) ifelse(x == "", NA, x))
  df2 <- data.frame(df2)
  df2 <- df2 %>% dplyr::mutate(OUTC_COD = as.factor(OUTC_COD))
  df2 <- table1(~OUTC_COD, data = df2)
  df2 <- data.frame(df2)
  write.xlsx(df2, file = file.path(path, "单独用药结局指标频数统计.xlsx"))
  datalib1.demo <- data.frame(datalib1.demo)
  datalib1.demo <- datalib1.demo %>% select(AGE, WT, SEX,
                                            OCCP_COD, REPORTER_COUNTRY, GetDataYear) %>% dplyr::mutate(GetDataYear = dplyr::case_when(41 <=
                                                                                                                                        GetDataYear & GetDataYear <= 44 ~ "2004年", 51 <= GetDataYear &
                                                                                                                                        GetDataYear <= 54 ~ "2005年", 61 <= GetDataYear & GetDataYear <=
                                                                                                                                        64 ~ "2006年", 71 <= GetDataYear & GetDataYear <= 74 ~
                                                                                                                                        "2007年", 81 <= GetDataYear & GetDataYear <= 84 ~ "2008年",
  ...
  datalib1.demo <- table1(~SEX + WT + AGE + OCCP_COD + REPORTER_COUNTRY,
                          data = datalib1.demo)
  datalib1.demo <- data.frame(datalib1.demo)
  write.xlsx(datalib1.demo, file = file.path(path, "单独用药基线信息整理汇总.xlsx"))
  target.drug <- inter.drug[(DRUGNAME %in% drug_PS | prod_ai %in%
                               drug_PS) & (ROLE_COD %in% drugEffect)]
  datalib1.demo <- inter.demo[unique(target.drug$primaryid)]
  datalib2.drug <- inter.drug[unique(datalib1.demo$primaryid)]
  ...
  if (!file.exists(path))
    dir.create(path, recur = T)
  data.table::fwrite(datalib1.demo, file = file.path(path,
                                                     paste0("不区分单独用药和联合用药demo.csv")))
  data.table::fwrite(datalib2.drug, file = file.path(path,
                                                     paste0("不区分单独用药和联合用药drug.csv")))
  data.table::fwrite(datalib2.outc, file = file.path(path,
                                                     paste0("不区分单独用药和联合用药outc.csv")))
  data.table::fwrite(datalib2.indi, file = file.path(path,
                                                     paste0("不区分单独用药和联合用药indi.csv")))
  data.table::fwrite(datalib2.reac, file = file.path(path,
                                                     paste0("不区分单独用药和联合用药reac.csv")))
  data.table::fwrite(datalib2.rpsr, file = file.path(path,
                                                     paste0("不区分单独用药和联合用药rpsr.csv")))
  data.table::fwrite(datalib2.ther, file = file.path(path,
                                                     paste0("不区分单独用药和联合用药ther.csv")))
  T1 <- as.data.frame(datalib1.demo)
  T2 <- as.data.frame(datalib2.drug)
  T2 <- T2 %>% dplyr::filter(ROLE_COD == "PS")
  T3 <- as.data.frame(datalib2.ther)
  ...
  T2 <- merge(T4, T1, by = "primaryid", all.x = T)
  T2 <- na.omit(T2)
  write.xlsx(T1, file = file.path(path, "不区分单独用药和联合用药不良反应诱发时间.xlsx"))
  write.xlsx(T2, file = file.path(path, "不区分单独用药和联合用药不良反应诱发时间(每个PT的不良反应诱发时间).xlsx"))
  ok <- datalib2.reac[, .(a = as.numeric(.N)), PT]
  ok[, `:=`(b, as.numeric(NROW(datalib2.reac)) - a)]
  ok1 <- inter.reac[, .(ac = as.numeric(.N)), PT]
  ok1[, `:=`(n, as.numeric(NROW(inter.reac)))]
  ...
  ok2$"EBGM(95%下限)" <- paste(ok2$EBGM, "(", ok2$EBGM05,
                             ")")
  ok2$"IC(95%下限)" <- paste(ok2$IC2, "(", ok2$C025, ")")
  write.xlsx(ok2, file = file.path(path, "不区分单独用药和联合用药不良反应信号(PT).xlsx"))
  okk <- datalib2.reac[, .(a = as.numeric(.N)), soc_name_cn]
  okk[, `:=`(b, as.numeric(NROW(datalib2.reac)) - a)]
  okk1 <- inter.reac[, .(ac = as.numeric(.N)), soc_name_cn]
  okk1[, `:=`(n, as.numeric(NROW(inter.reac)))]
  ...
  okk2$"IC(95%下限)" <- paste(okk2$IC2, "(", okk2$C025,
                            ")")
  write.xlsx(okk2, file = file.path(path, "不区分单独用药和联合用药不良反应信号(soc).xlsx"))
  okkk <- datalib2.reac[, .(a = as.numeric(.N)), hlt_name_cn]
  okkk[, `:=`(b, as.numeric(NROW(datalib2.reac)) - a)]
  okkk1 <- inter.reac[, .(ac = as.numeric(.N)), hlt_name_cn]
  okkk1[, `:=`(n, as.numeric(NROW(inter.reac)))]
  ...
  okkk2$"IC(95%下限)" <- paste(okkk2$IC2, "(", okkk2$C025,
                             ")")
  write.xlsx(okkk2, file = file.path(path, "不区分单独用药和联合用药不良反应信号(hlt).xlsx"))
  okkkk <- datalib2.reac[, .(a = as.numeric(.N)), hlgt_name_cn]
  okkkk[, `:=`(b, as.numeric(NROW(datalib2.reac)) - a)]
  okkkk1 <- inter.reac[, .(ac = as.numeric(.N)), hlgt_name_cn]
  okkkk1[, `:=`(n, as.numeric(NROW(inter.reac)))]
  ...
  okkkk2 <- data.frame(okkkk2)
  okkkk2 <- okkkk2 %>% dplyr::mutate_if(is.numeric, round,
                                        digits = 2)
  okkkk2$"ROR(95%Cl)" <- paste(okkkk2$ROR, "(", okkkk2$RORL,
                               "-", okkkk2$RORU, ")")
  okkkk2$"PRR(卡方值)" <- paste(okkkk2$PRR, "(", okkkk2$XX,
  ...
  okkkk2$"IC(95%下限)" <- paste(okkkk2$IC2, "(", okkkk2$C025,
                              ")")
  write.xlsx(okkkk2, file = file.path(path, "不区分单独用药和联合用药不良反应信号(hlgt).xlsx"))
  df1 <- datalib1.demo[, `:=`(GetDataYear, dplyr::case_when(41 <=
                                                              GetDataYear & GetDataYear <= 44 ~ "2004年", 51 <= GetDataYear &
                                                              GetDataYear <= 54 ~ "2005年", 61 <= GetDataYear & GetDataYear <=
                                                              64 ~ "2006年", 71 <= GetDataYear & GetDataYear <= 74 ~
  ...
  p1 <- ggplot(df1, aes(x = GetDataYear, y = Number)) + geom_bar(stat = "identity",
                                                                 width = 0.4) + xlab("") + theme_bw()
  ggsave(file.path(path, "不区分单独用药和联合用药上报年份图.png"),
         p1, width = 20, height = 8.27, dpi = 600)
  write.xlsx(df1, file = file.path(path, "不区分单独用药和联合用药季度上报频数统计.xlsx"))
  df2 <- data.frame(datalib2.outc)
  df2 <- apply(df2, 2, function(x) ifelse(x == "", NA, x))
  df2 <- data.frame(df2)
  df2 <- df2 %>% dplyr::mutate(OUTC_COD = as.factor(OUTC_COD))
  df2 <- table1(~OUTC_COD, data = df2)
  df2 <- data.frame(df2)
  write.xlsx(df2, file = file.path(path, "不区分单独用药和联合用药结局指标频数统计.xlsx"))
  datalib1.demo <- data.frame(datalib1.demo)
  datalib1.demo <- datalib1.demo %>% select(AGE, WT, SEX,
                                            OCCP_COD, REPORTER_COUNTRY, GetDataYear) %>% dplyr::mutate(GetDataYear = dplyr::case_when(41 <=
                                                                                                                                        GetDataYear & GetDataYear <= 44 ~ "2004年", 51 <= GetDataYear &
  ...
  datalib1.demo <- table1(~SEX + WT + AGE + OCCP_COD + REPORTER_COUNTRY,
                          data = datalib1.demo)
  datalib1.demo <- data.frame(datalib1.demo)
  write.xlsx(datalib1.demo, file = file.path(path, "不区分单独用药和联合用药基线信息整理汇总.xlsx"))
