...
#'
#' @examples
GFun2 = function (age, gender, weight, agent, country, drugEffect, year,
                   illnesses, path)
{
  ...
  QQQQQ <- data.table(read.csv(paste(getwd()(),
                                     "特定primaryid号.csv", sep = "/")))
  QQQQQ <- QQQQQ[, `:=`(primaryid, as.character(primaryid))]
  data.table::setkeyv(QQQQQ, "primaryid")
  datalib1.demo <- inter.demo[unique(QQQQQ$primaryid)]
  ...
  okk2$"IC(95%下限)" <- paste(okk2$IC2, "(", okk2$C025,
                            ")")
  write.xlsx(okk2, file = file.path(path, "不良反应信号(soc).xlsx"))
  ...
  okkk2$"IC(95%下限)" <- paste(okkk2$IC2, "(", okkk2$C025,
                             ")")
  write.xlsx(okkk2, file = file.path(path, "不良反应信号(hlt).xlsx"))
  ...
  okkkk2$"IC(95%下限)" <- paste(okkkk2$IC2, "(", okkkk2$C025,
                              ")")
  write.xlsx(okkkk2, file = file.path(path, "不良反应信号(hlgt).xlsx"))
  ...
  ggsave(file.path(path, "上报年份图.png"), p1, width = 20,
         height = 8.27, dpi = 600)
  ...
  df2 <- table1::table1(~OUTC_COD, data = df2)
  df2 <- data.frame(df2)
